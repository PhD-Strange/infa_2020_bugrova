...
def Cat(n, x, y, eye_color0, color0, flipbool):
    ...
    its axis if you set the value "True" for the "flipbool" variable. The "n"
    parameter sets the size is relative to the original size
...
def whiskeys(surface1, surface2, surface3, surface4):
    """
    The function draws the cat's whiskers. Each of the 3 whiskers is drawn on
    its own surface, then compressed and transferred to the "surface4".
...
def Ball(surface, n, x, y, color, flipbool):
    ...
    to "True" or "False". The "n" parameter sets the size relative to
    the original size in the original image.
...
def Window(surface, n, x, y, color1, color2):
    ...
    The "n" parameter sets the size relative to the original size
    in the original image.
...
for functions was written. I shuffled the lines a little, but it's
not very significant. All surfaces used are set at the beginning, before
declaring functions. I added a little bit of PEP8 in the code.
